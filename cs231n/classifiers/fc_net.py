...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        ############################################################################
        # trying dividing by sqrt of input size
        if use_smart_w1_init == True:
            # reLU divide by sqrt(2/fan_in)  works well but kills structure in visualized W1 matrix
            self.params['W1'] = std * np.random.randn(input_dim, hidden_dim) / mth.sqrt(2.0 / input_dim)

            # divide by sqrt(fan_in) for non reLu  works well but kills structure in visualized W1 matrix
            self.params['W2'] = std * np.random.randn(hidden_dim, num_classes) / mth.sqrt(hidden_dim)
        else:
            # use reccomended by assign
    ...
    def loss(self, X, y=None):
        ...
        - scores: Array of shape (N, C) giving classification scores, where
          ...
          Note this is just the output of the final affine layer, no softmax
        ...
        ############################################################################
        outaf1, cache1 = affine_forward(X, self.params['W1'], self.params['b1'])
        outrl1, _ = relu_forward(outaf1)
        # out, cache1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])

        scores, cache2 = affine_forward(outrl1, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, dw2, db2 = affine_backward(dsm, cache2)

        grads['W2'] = dw2 + self.reg * self.params['W2']
        grads['b2'] = db2

        dx = relu_backward(dx, cache2[0])
